...
export default function EditShop() {
  ...
  useEffect(() => {
    ...
    read({ shopId: params.shopId }, { t: jwt.token }, signal).then((data) => {
      if (data.error) {
        // Redirect if user is not authenticated or authorized
        if (
          data.error.toLowerCase().includes("not authorized") ||
          data.error.toLowerCase().includes("not signed in") ||
          data.error.toLowerCase().includes("unauthorized")
        ) {
          setValues((prev) => ({ ...prev, redirect: "signin" }));
        } else {
          setValues((prev) => ({ ...prev, error: data.error }));
        }
      } else {
    ...
    update({ shopId: params.shopId }, { t: jwt.token }, shopData).then(
      (data) => {
        if (data.error) {
          ...
          setValues((prev) => ({ ...prev, redirect: "shops" }));
        }
      }
    );
  };
  ...
  // Handle redirections
  if (values.redirect === "signin") {
    return <Navigate to="/signin" />;
  }

  if (values.redirect === "shops") {
    return <Navigate to="/seller/shops" />;
  }
